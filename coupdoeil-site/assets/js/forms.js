  ...
  const handleArtistForm = () => {
    ...
    form.addEventListener("submit", async (e) => {
      ...
      ["firstname", "lastname", "discipline"].forEach((field) => {
        if (!secure(data.get(field)))
          errors.push(`Champ ${field} obligatoire.`);
      });
      if (!form.querySelector('[name="consent"]:checked'))
        errors.push("Le consentement est requis.");
      if (errors.length) {
      ...
      const artist = {
        ...
        logistics: Array.from(
          form.querySelectorAll('[name="logistics"]:checked')
        ).map((i) => i.value),
        images: preview
      ...
      const payload = {
        ...
        logistics: artist.logistics.length
          ? artist.logistics.join(", ")
          : "Non précisé",
        visuals: artist.images.length
          ? `${artist.images.length} visuel(s) ajoutés (aperçu local uniquement)`
          : "Aucun visuel joint",
      };
      try {
        ...
        const stored = JSON.parse(
          localStorage.getItem("coupdoeil_artists") || "[]"
        );
        stored.push(artist);
        ...
        setStatus(
          form,
          "Merci ! Votre candidature a été transmise et enregistrée localement."
        );
      } catch (error) {
        console.error("Artist form submission failed", error);
        setStatus(
          form,
          "Impossible d'envoyer le formulaire. Merci de réessayer dans un instant.",
          true
        );
      }
    });
  ...
  const handleVisitorForm = () => {
    ...
    form.addEventListener("submit", async (e) => {
      ...
      const visits = Array.from(
        form.querySelectorAll('[name="editions"]:checked')
      ).map((i) => i.value);
      const wantsNewsletter = !!form.querySelector(
      ...
      try {
        ...
        localStorage.setItem(
          "coupdoeil_visitors",
          JSON.stringify({ name, email, visits })
        );
        form.reset();
        setStatus(
          form,
          "Merci ! Votre inscription visiteur est transmise et mémorisée localement."
        );
      } catch (error) {
        console.error("Visitor form submission failed", error);
        setStatus(
          form,
          "Impossible d'envoyer votre inscription pour le moment. Réessayez ou contactez l'équipe.",
          true
        );
      }
    });
